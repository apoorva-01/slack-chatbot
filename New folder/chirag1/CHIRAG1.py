...
def get_thread_messages(channel, thread_ts):
    try:
        ...
        for msg in messages:
            ...
            if not is_bot:
                # User message
                formatted_output.append(f"User Query — {text}".replace(f"<@{SLACK_BOT_USER_ID}>", "").strip())
            elif metadata.get("event_type") == "tracking_point":
                # Bot message with specific metadata
                formatted_output.append(f"Assistant Reply — {text}".strip())
                pass
...
5. Questions About Project Status (e.g., “Which projects are in progress?” or “What’s currently happening?”)
✅ DO THIS:
When asked about status-based queries, follow this logic:
⸻
🔍 Step 1: Check the Status field
	•	If the Status includes terms like:
	•	“In Progress”
	•	“Lead Dev Review”
	•	“QA In Progress”
	•	“Pending”
	...
	•	“Design Phase”
	•	etc.
👉 Consider the project as “In Progress”
	•	If the Status indicates finality (e.g., “Deployed”, “Completed”, “Cancelled”), do not consider it in progress.
🔍 Step 2: Validate with Created Time and Deployment Date
	•	If the Deployment Date is:
	•	Missing, N/A, or in the future,
→ and the Status matches any of the in-progress indicators above,
✅ Confirm it as “in progress”.
	•	If the Deployment Date exists and is in the past, but the Status still shows “In Progress”,
...
▶️ If the user asks for **project details**, “[project name]”, “summary”, or similar:

Return the **FULL structured response**, in this format:
...
▶️ If the user query is **specific** (e.g. “What are the Dev hours for Project X?”, “Any comments on Project X?”, “Deployment date?” etc):

✅ Return **ONLY the relevant fields** in a compact format. Example:

**Dev Hours for Project X**: 32 hours  
**Deployment Date**: March 15, 2024  
**Comments**: Waiting for client feedback  

⚠️ DO NOT return the full structured block for specific field queries.

---

🧠 For queries about **scope, goals, blockers, challenges, progress notes, or team decisions**, summarize ONLY the relevant points from:
- 🧾 HubSpot notes
- 📞 Meeting transcripts
- 💬 Slack messages
...
🚫 Never invent info. Use only what’s available in structured or unstructured context.
...
async def process_faiss_and_generate_responses(user_query, thread_context, assistant_name, channel, thread_ts,query_type,project_name):
    relevant_chunks = await async_faiss_search(user_query + thread_context,  assistant_name) or ["No relevant data found."]
